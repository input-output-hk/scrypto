...
class AVLTreeSpecification extends PropSpec with GeneratorDrivenPropertyChecks with TwoPartyTests {
  ...
  //      println(Base58.encode(aKey))
        ...
        if(l.verifyLookup(digest, existence = true).isEmpty) {
          println("====")
          l.proofSeq.foreach( p => println(p.getClass.getSimpleName))
          l.verifyLookup(digest, existence = true)
        }
